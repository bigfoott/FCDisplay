...
namespace FCDisplay
{
    class DisplayObject : MonoBehaviour
    {
        private Image img;
        ...
        private void Init()
        {
            ...
            img.color = new Color(
                Plugin.Config.GetInt("FCDisplay", "ColorRed", 255, true),
                Plugin.Config.GetInt("FCDisplay", "ColorGreen", 255, true),
                Plugin.Config.GetInt("FCDisplay", "ColorBlue", 255, true));

            g.transform.position = new Vector3(3.2f, 1.5f, 7f);
            
            if (!Plugin.Config.GetBool("FCDisplay", "VanillaEnabled", false, true))
            {
        ...
        private IEnumerator Missed()
        {
            if (!startedDelete)
            {
                ...
                string mode = Plugin.effects[Plugin.Config.GetInt("FCDisplay", "MissEffect", 1)];
